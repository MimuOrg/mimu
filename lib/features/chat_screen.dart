...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  ...
  Future<void> _addImageMessage(String path, {String? caption}) async {
    ...
    // Создаем сообщение локально сразу для быстрого отображения
    final message = ChatMessage(
      id: now.millisecondsSinceEpoch.toString(),
    ...
    // Загрузка на сервер происходит автоматически в ChatStore._sendMessageToServer
  }
  ...
  void _showCaptionDialog(String path,
    ...
    showCupertinoDialog(
      ...
      builder: (context) => buildCupertinoDialog(
        context: context,
        title: isImage ? 'Добавить подпись к фото' : 'Добавить подпись к файлу',
        content: CupertinoTextField(
          controller: captionController,
          placeholder: isImage ? 'Подпись (необязательно)' : 'Подпись (необязательно)',
          maxLines: 3,
          autofocus: true,
          style: const TextStyle(color: CupertinoColors.white),
          decoration: BoxDecoration(
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const Spacer(),
                    IconButton(
                      icon: const Icon(CupertinoIcons.xmark, weight: 700),
  ...
  Widget build(BuildContext context) {
    return Consumer<ChatStore>(
      builder: (context, chatStore, _) {
        ...
        final contactId = !chat.isGroup && chat.participantIds.isNotEmpty
            ? chat.participantIds.first
            : null;
        final isBlocked =
            contactId != null && chatStore.isContactBlocked(contactId);
        final isIgnored =
            contactId != null && chatStore.isContactIgnored(contactId);

        final messages = isIgnored && !chat.isGroup
            ? chat.messages.where((m) => m.isMe).toList()
            : chat.messages;
                ...
                : [
                    Builder(
                      builder: (buttonContext) => IconButton(
                        splashRadius: 22,
                        icon: const Icon(CupertinoIcons.ellipsis_vertical, weight: 700),
                        onPressed: () {
                          Navigator.of(context).push(NavigationService.createSlideTransitionRoute(ProfileScreen(userName: chat.title, avatarAsset: chat.avatarAsset)));
                        },
                      ),
                    )
                  ],
                          ...
                          children: [
                            ...
                            GestureDetector(
                              ...
                              child: Dismissible(
                                ...
                                .animate(delay: (20 * messageIndex).ms)
                                .fadeIn(
                                    duration: 250.ms,
                                    curve: Curves.easeOutCubic)
                                .slideY(
                          ...
                          children: [
                            if (isBlocked)
                              Padding(
                                ...
                                child: Text(
                                  'Контакт заблокирован — отправка отключена',
                                  style: TextStyle(
  ...
  void _showCustomMenu(
    ...
    showMenu<String>(
      ...
      items: [
        _buildMenuItem('call', 'Звонок', CupertinoIcons.phone),
        _buildMenuItem('video', 'Видеозвонок', CupertinoIcons.videocam),
        const PopupMenuDivider(),
        _buildMenuItem(
            'search', 'Поиск в чате', CupertinoIcons.search),
        _buildMenuItem('media', 'Медиафайлы', CupertinoIcons.photo),
        _buildMenuItem('files', 'Файлы', CupertinoIcons.doc),
        const PopupMenuDivider(),
        _buildMenuItem(
            'mute', 'Отключить уведомления', CupertinoIcons.bell_slash),
        _buildMenuItem('pin', 'Закрепить чат', CupertinoIcons.pin),
        _buildMenuItem('clear', 'Очистить историю', CupertinoIcons.delete,
            isDestructive: true),
      ],
    ).then((value) {
      setState(() => _isDialogOpen = false);
  ...
  Future<void> _showImagePicker(ImageSource source) async {
    try {
      ...
      if (mounted) {
        Navigator.of(context).pop(); // Закрываем индикатор при ошибке
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Telegram iOS стиль - чистые цвета без градиентов
    final userBubbleColor = Theme.of(context).primaryColor; // Акцентный цвет приложения
    final interlocutorBubbleColor = const Color(0xFF2C2C2E); // Telegram iOS серый
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          margin: EdgeInsets.only(
            ...
            top: SettingsService.getCompactMode() ? 0.5 : 1.5,
            bottom: SettingsService.getCompactMode() ? 0.5 : 1.5,
          ),
          child: message.isMe
  ...
  Widget _buildUserBubble(BuildContext context, Color color, ThemeData theme) {
    // Telegram iOS стиль - простой цвет без градиентов
    final bubbleRadius = 18.0; // Стандартный радиус Telegram iOS
    
    final decoration = BoxDecoration(
      color: color, // Акцентный цвет приложения
      borderRadius: BorderRadius.only(
        topLeft: Radius.circular(bubbleRadius),
        topRight: Radius.circular(4), // Маленький угол для "хвостика"
    ...
    return Container(
      ...
      padding: EdgeInsets.fromLTRB(
        10,
        SettingsService.getCompactMode() ? 6 : 8,
        10,
        SettingsService.getCompactMode() ? 6 : 8,
      ),
      child: Column(
  ...
  Widget _buildInterlocutorBubble(
    ...
    // Telegram iOS стиль - серый цвет для сообщений собеседника
    final bubbleRadius = 18.0; // Стандартный радиус Telegram iOS
    
    final decoration = BoxDecoration(
      color: const Color(0xFF2C2C2E), // Telegram iOS серый
      borderRadius: BorderRadius.only(
        topLeft: Radius.circular(4), // Маленький угол для "хвостика"
      ...
      builder: (context) => buildCupertinoActionSheet(
        ...
        actions: [
          ...
          CupertinoActionSheetAction(
            ...
            child: const Text('Добавить реакцию'),
          ),
          if (message.type == ChatMessageType.text)
            CupertinoActionSheetAction(
              onPressed: () {
                        ...
                        child: (message.mediaPath != null &&
                            ...
                            ? (File(message.mediaPath!).existsSync()
                                ...
                                : Image.asset('assets/images/avatar_placeholder.png',
                                    fit: BoxFit.contain))
                            : Image.asset(message.mediaPath ?? 'assets/images/avatar_placeholder.png',
                                fit: BoxFit.contain),
                      ),
                    ),
                  ),
                      ...
                      child: Row(
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: widget.enabled ? widget.onCancelReply : null,
                            padding: EdgeInsets.zero,
                            ...
                            children: [
                              ...
                              GlassContainer(
                                ...
                                child: Material(
                                  ...
                                  child: InkWell(
                                    borderRadius: BorderRadius.circular(10),
                                    onTap: widget.enabled ? () => widget.onToggleRecord() : null,
                                    onLongPress: widget.enabled ? () => widget.onToggleRecord() : null,
                                    child: Container(
                                      width: 36,
                                      height: 36,
                                      alignment: Alignment.center,
