...
class _AppearanceSettingsState extends State<_AppearanceSettings> {
  ...
  Widget build(BuildContext context) {
    ...
    // Богатая черная палитра вместо фиолетовых цветов
    final accentColors = [
      const Color(0xFF000000), // Rich Black
      const Color(0xFF050505), // Deep Black
      const Color(0xFF0A0A0A), // Charcoal Black
      const Color(0xFF0F0F0F), // Dark Charcoal
      const Color(0xFF1A1A1A), // Medium Black
      const Color(0xFF252525), // Light Black
      const Color(0xFF2A2A2A), // Soft Black
      const Color(0xFF303030), // Muted Black
      const Color(0xFF00BCD4), // Cyan (для контраста)
      const Color(0xFF4CAF50), // Green (для контраста)
      const Color(0xFFFF9800), // Orange (для контраста)
      const Color(0xFF2196F3), // Blue (для контраста)
      const Color(0xFFF44336), // Red (для контраста)
      const Color(0xFF00E676), // Light Green (для контраста)
      const Color(0xFFFFC107), // Amber (для контраста)
      const Color(0xFF3F51B5), // Indigo (для контраста)
    ];
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                // Богатые черные цвета для разных тем
                Color themeColor;
                switch (theme) {
                  case 'Mimu Classical':
                    themeColor = const Color(0xFF0A0A0A); // Charcoal Black
                    break;
                  case 'Winter Ocean':
                    themeColor = const Color(0xFF0F0F0F); // Dark Charcoal
                    break;
                  case 'Melanholic':
                    themeColor = const Color(0xFF050505); // Deep Black
                    break;
                  case 'Dark Mode':
                    themeColor = const Color(0xFF000000); // Rich Black
                    break;
                  case 'Light Mode':
                    themeColor = const Color(0xFF1A1A1A); // Medium Black
                    break;
                  case 'Amoled Black':
                    themeColor = const Color(0xFF000000); // Pure Black for AMOLED
                    break;
                  case 'Ocean Blue':
                    themeColor = const Color(0xFF0A0A0A); // Charcoal Black
                    break;
          ...
          GlassContainer(
            ...
            child: Column(
              children: [
                ...
                Divider(height: 1, color: Colors.white.withOpacity(0.1)),
                CupertinoFormRow(
                  prefix: const Text('Анимации'),
                  child: CupertinoPicker(
                    itemExtent: 32,
                    scrollController: FixedExtentScrollController(
                      initialItem: SettingsService.getAnimationsEnabled() ? 0 : 1,
                    ),
                    onSelectedItemChanged: (index) async {
                      await SettingsService.setAnimationsEnabled(index == 0);
                      setState(() {});
                    },
                    children: const [
                      Text('Включены'),
        ...
        children: [
          ...
          GlassContainer(
            ...
            child: DropdownButton<String>(
              value: _audioInput,
              isExpanded: true,
              dropdownColor: const Color(0xFF0A0A0A), // Богатый черный вместо фиолетового
